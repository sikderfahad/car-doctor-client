...
export default function OurTeam() {
  const { data: experts, isLoading } = useLoadData("/team-members");
            ...
            {experts?.data &&
              experts?.data.slice(index, index + visibleCards).map((expert) => (
                <motion.div
                  ...
                  initial={{ opacity: 0, x: visibleCards < 3 ? 100 : 500 }}
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: visibleCards < 3 ? -100 : -500 }}
                  transition={{ duration: visibleCards < 3 ? 0.2 : 0.5 }}
                >
